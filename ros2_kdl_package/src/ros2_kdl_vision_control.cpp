...
class Iiwa_pub_sub : public rclcpp::Node
{
    public:
        ...
        node_handle_(std::shared_ptr<Iiwa_pub_sub>(this))
        {
            ...
            transform_subscriber_ = this->create_subscription<geometry_msgs::msg::TransformStamped>(
                "transform_pub", 10, std::bind(&Iiwa_pub_sub::transformCallback, this, std::placeholders::_1));
    ...
    private:
        ...
        void transformCallback(const geometry_msgs::msg::TransformStamped::SharedPtr msg){
            // Estrai la traslazione (posizione) dal messaggio di trasformazione
            Eigen::Vector3d position(msg->transform.translation.x, msg->transform.translation.y, msg->transform.translation.z);

            // Aggiungi un offset di 10 cm sull'asse X
            position.x() += 0.1;  // 10 cm in più sull'asse X

            // Ora 'position' contiene la nuova posizione (con l'offset sull'asse X)
            end_position = position;

            // Stampa la posizione finale per verificare
            RCLCPP_INFO(this->get_logger(), "End position with offset: [%f, %f, %f]", end_position.x(), end_position.y(), end_position.z());
            end_position_available_ = true;
        }

        rclcpp::Subscription<sensor_msgs::msg::JointState>::SharedPtr jointSubscriber_;
        rclcpp::Subscription<geometry_msgs::msg::TransformStamped>::SharedPtr transform_subscriber_;
